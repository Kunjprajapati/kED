...
def apply_logic_FOR_AR_CMD(pathing1, pathing2):
    data = pd.read_excel(pathing1)
    date = datetime.date.today() 
    data_Dictionary = pd.read_excel(pathing2, sheet_name='page')
    # data_npi is removed and replaced by npi_dict
    npi_dict = {
        "Ash, Zavier": 1770569204,
        "Isom, Joel": 1194930222,
        "Nwaigwe, Odinachi": 1144955907,
        "Guidry, Karen": 1346864725,
        "Babalola, Adeniyi": 1497793970,
        "Williamson, Natasha": 1386033132,
        "Mayberry, Tracy": 1497738413,
        "Duker, Adjoa": 1881713121,
        "Roberson, Audrey C": 1811495385,
        "Kibuyaga Travis, Nakato B": 1609031657,
        "Smith, Ethel L": 1104817014,
        "Tafari, Solomon": 1235128828,
        "Collins, Darrice": 1760802268
    }
    ...
    data['Payer Subscriber No'] = data['Payer Subscriber No'].fillna("NaN")
    ...
    filtered_data.loc[:, 'Patient First Name'] = data['Patient Name'].apply(lambda x: x.split(",")[0])
    filtered_data.loc[:, 'Patient Last Name'] = data['Patient Name'].apply(lambda x: x.split(",")[1])

    filtered_data.loc[:, 'Subscriber First Name'] = data['Patient Name'].apply(lambda x: x.split(",")[0])
    filtered_data.loc[:, 'Subscriber Last Name'] = data['Patient Name'].apply(lambda x: x.split(",")[1])
    ...
    resultant_data["Provider Group NPI"] = 1902407752
    resultant_data["Provider Group Name"] = "Community MD"

    # List to store paths of all generated Excel files
    generated_files = []

    # Ensure the directory exists before saving the Excel file
    output_directory = "Results/AR/AR_CMD/Reference Files"
    ensure_directory_exists(output_directory)
    output_path = f"{output_directory}/Reference_AR_CMD_File.xlsx"
    ...
    # Desized Column Ordering with respect to RAIO CMD AR
    ...
    ensure_directory_exists('Results/AR/AR_CMD/Insurances_Above_120')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_above_120:
        ...
        if not group_same.empty:
            filename_same = "CMD_120+_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_CMD/Insurances_Above_120/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Above_120/{filename_same}')

        # Processing the 'Different' group
        if not group_different.empty:
            filename_different = "CMD_120+_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_CMD/Insurances_Above_120/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Above_120/{filename_different}')
    ...
    ensure_directory_exists('Results/AR/AR_CMD/Insurances_Between_91_To_120')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_91_to_120:
        ...
        if not group_same.empty:
            filename_same = "CMD_91_To_120_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_CMD/Insurances_Between_91_To_120/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Between_91_To_120/{filename_same}')

        if not group_different.empty:
            filename_different = "CMD_91_To_120_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_CMD/Insurances_Between_91_To_120/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Between_91_To_120/{filename_different}')
    ...
    ensure_directory_exists('Results/AR/AR_CMD/Insurances_Between_61_To_90')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_61_to_90:
        ...
        if not group_same.empty:
            filename_same = "CMD_61_To_90_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_CMD/Insurances_Between_61_To_90/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Between_61_To_90/{filename_same}')

        if not group_different.empty:
            filename_different = "CMD_61_To_90_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_CMD/Insurances_Between_61_To_90/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Between_61_To_90/{filename_different}')
    ...
    ensure_directory_exists('Results/AR/AR_CMD/Insurances_Between_31_To_60')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_31_to_60:
        ...
        if not group_same.empty:
            filename_same = "CMD_31_To_60_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_CMD/Insurances_Between_31_To_60/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Between_31_To_60/{filename_same}')

        if not group_different.empty:
            filename_different = "CMD_31_To_60_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_CMD/Insurances_Between_31_To_60/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Between_31_To_60/{filename_different}')





    # Dropping the 'Number of Days' column from resultant_data_less_    resultant_data_less_30 = resultant_data_less_30.drop(columns=['Number of Days'])
    resultant_data_less_30 = resultant_data_less_30[columns_arrangment]
    ensure_directory_exists('Results/AR/AR_CMD/Insurances_Below_30')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_less_30:
        ...
        if not group_same.empty:
            filename_same = "CMD_30-_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_CMD/Insurances_Below_30/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Below_30/{filename_same}')

        if not group_different.empty:
            filename_different = "CMD_30-_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_CMD/Insurances_Below_30/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_CMD/Insurances_Below_30/{filename_different}')


    ensure_directory_exists("Results/AR/AR_CMD/Reference Files")
    resultant_data_above_120.to_excel('Results/AR/AR_CMD/Reference Files/Data_Above_120.xlsx', index = False)
    generated_files.append('Results/AR/AR_CMD/Reference Files/Data_Above_120.xlsx')  # Append file path to generated_files
    resultant_data_between_91_and_120.to_excel('Results/AR/AR_CMD/Reference Files/Data_Between_91_To_120.xlsx', index = False)
    generated_files.append('Results/AR/AR_CMD/Reference Files/Data_Between_91_To_120.xlsx')  # Append file path to generated_files
    resultant_data_between_61_and_90.to_excel('Results/AR/AR_CMD/Reference Files/Data_Between_61_To_90.xlsx', index = False)
    generated_files.append('Results/AR/AR_CMD/Reference Files/Data_Between_61_To_90.xlsx')  # Append file path to generated_files
    resultant_data_between_31_and_60.to_excel('Results/AR/AR_CMD/Reference Files/Data_Between_31_To_60.xlsx', index = False)
    generated_files.append('Results/AR/AR_CMD/Reference Files/Data_Between_31_To_60.xlsx')  # Append file path to generated_files
    resultant_data_less_30.to_excel('Results/AR/AR_CMD/Reference Files/Data_Less_30.xlsx', index = False)
    generated_files.append('Results/AR/AR_CMD/Reference Files/Data_Less_30.xlsx')  # Append file path to generated_files

    # Create a ZIP file to store all result files
    zip_file_path = 'temp/AR_CMD_output_files.zip'
    os.makedirs(os.path.dirname(zip_file_path), exist_ok=True)
    with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for file_path in generated_files:
            archive_path = os.path.relpath(file_path, 'Results/AR/AR_CMD')
