...
def apply_logic_FOR_AR_Gulf(pathing1, pathing2):
    generated_files = []  # Initialize the list to store file paths
    ...
    npi_dict = {
    "Kapadia, Manish K": 1205865615, 
    "Patel, Rajeshri" : 1366503344
    }
    ...
    #     # formated_data.to_excel("Results/AR/AR_Gulf/_Sample___AR___Gulf_.xlsx", index=False)
    # else:
    #     print([resultant_data['CPT Code'].last_valid_index()])
    #     print([resultant_data['Provider First Name'].last_valid_index()])
    #     print([resultant_data['Provider Last Name'].last_valid_index()])
    ...
    resultant_data["Provider Group NPI"] = 1851478994
    resultant_data["Provider Group Name"] = "Gulf Coast Pulmonary Medicine PA"

    # Ensure the directory exists before saving the Excel file
    ensure_directory_exists("Results/AR/AR_Gulf/Reference Files")
    resultant_data.to_excel("Results/AR/AR_Gulf/Reference Files/Reference_AR_Gulf_File.xlsx", index=False)
    generated_files.append("Results/AR/AR_Gulf/Reference Files/Reference_AR_Gulf_File.xlsx")  # Append file path to generated_files
    ...
    # Desized Column Ordering with respect to RAIO Gulf AR
    ...
    ensure_directory_exists('Results/AR/AR_Gulf/Insurances_Above_120')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_above_120:
        ...
        if not group_same.empty:
            filename_same = "Gulf_120+_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gulf/Insurances_Above_120/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Above_120/{filename_same}')  # Append file path to generated_files

        # Processing the 'Different' group
        if not group_different.empty:
            filename_different = "Gulf_120+_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gulf/Insurances_Above_120/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Above_120/{filename_different}')  # Append file path to generated_files
    ...
    ensure_directory_exists('Results/AR/AR_Gulf/Insurances_Between_91_To_120')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_91_to_120:
        ...
        if not group_same.empty:
            filename_same = "Gulf_91_To_120_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gulf/Insurances_Between_91_To_120/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Between_91_To_120/{filename_same}')  # Append file path to generated_files

        if not group_different.empty:
            filename_different = "Gulf_91_To_120_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gulf/Insurances_Between_91_To_120/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Between_91_To_120/{filename_different}')  # Append file path to generated_files
    ...
    ensure_directory_exists('Results/AR/AR_Gulf/Insurances_Between_61_To_90')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_61_to_90:
        ...
        if not group_same.empty:
            filename_same = "Gulf_61_To_90_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gulf/Insurances_Between_61_To_90/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Between_61_To_90/{filename_same}')  # Append file path to generated_files

        if not group_different.empty:
            filename_different = "Gulf_61_To_90_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gulf/Insurances_Between_61_To_90/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Between_61_To_90/{filename_different}')  # Append file path to generated_files
    ...
    ensure_directory_exists('Results/AR/AR_Gulf/Insurances_Between_31_To_60')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_31_to_60:
        ...
        group_different = group[group['Charges Amount'] != group['Total Outstanding Balance']]
        if not group_same.empty:
            filename_same = "Gulf_31_To_60_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gulf/Insurances_Between_31_To_60/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Between_31_To_60/{filename_same}')  # Append file path to generated_files

        if not group_different.empty:
            filename_different = "Gulf_31_To_60_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gulf/Insurances_Between_31_To_60/{filename_different}', index=False)
            counter += 1
            print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Between_31_To_60/{filename_different}')  # Append file path to generated_files
    ...
    ensure_directory_exists('Results/AR/AR_Gulf/Insurances_Below_30')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_less_30:
        ...
        if not group_same.empty:
            filename_same = "Gulf_30-_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gulf/Insurances_Below_30/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Below_30/{filename_same}')  # Append file path to generated_files

        if not group_different.empty:
            filename_different = "Gulf_30-_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gulf/Insurances_Below_30/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gulf/Insurances_Below_30/{filename_different}')  # Append file path to generated_files


    # Ensure the directory exists before saving the Excel file
    ensure_directory_exists('Results/AR/AR_Gulf/Reference Files')  # Add this line
    resultant_data_above_120.to_excel('Results/AR/AR_Gulf/Reference Files/Data_Above_120.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gulf/Reference Files/Data_Above_120.xlsx')  # Append file path to generated_files
    resultant_data_between_91_and_120.to_excel('Results/AR/AR_Gulf/Reference Files/Data_Between_91_To_120.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gulf/Reference Files/Data_Between_91_To_120.xlsx')  # Append file path to generated_files
    resultant_data_between_61_and_90.to_excel('Results/AR/AR_Gulf/Reference Files/Data_Between_61_To_90.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gulf/Reference Files/Data_Between_61_To_90.xlsx')  # Append file path to generated_files
    resultant_data_between_31_and_60.to_excel('Results/AR/AR_Gulf/Reference Files/Data_Between_31_To_60.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gulf/Reference Files/Data_Between_31_To_60.xlsx')  # Append file path to generated_files
    resultant_data_less_30.to_excel('Results/AR/AR_Gulf/Reference Files/Data_Less_30.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gulf/Reference Files/Data_Less_30.xlsx')  # Append file path to generated_files


    # Create a ZIP file to store all result files
    zip_file_path = 'temp/AR_Gulf_output_files.zip'
    os.makedirs(os.path.dirname(zip_file_path), exist_ok=True)
    with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for file_path in generated_files:
            archive_path = os.path.relpath(file_path, 'Results/AR/AR_Gulf')
...
# print(AR_Gulf_Preprocessing(pathing = "31.09 - Payer Claim Aging - Detail (5).xlsx"))
