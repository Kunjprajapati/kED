...
# data = pd.read_excel('SWMD Charges of 06.20.2024.xlsx')

import warnings
warnings.filterwarnings('ignore', category=pd.errors.SettingWithCopyWarning)
warnings.filterwarnings('ignore', category=FutureWarning)  

def function_for_SWMD(pathing):
    ...
    # result.to_excel("SWMD Charges of 06202024.xlsx", index=0)
    # return result
    output_dir = 'Results/DailyCharges/SWMD'
    os.makedirs(output_dir, exist_ok=True)
    filename = os.path.basename(pathing)
    filename_no_ext = os.path.splitext(filename)[0]
    parts = filename_no_ext.split('_')
    kunjdate = parts[-1] 
    output_path = os.path.join(output_dir, f"SWMD_charges_of_{kunjdate}.xlsx")
