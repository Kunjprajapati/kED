...
def function_for_coolkidz(pathing):
    ...
    output_dir = 'Results/DailyCharges/CoolKidz'
    os.makedirs(output_dir, exist_ok=True)
    # output_path = os.path.join(output_dir, '[KUNJ]_Coolkidz.xlsx')
    # kunj = list(pathing.split("/"))
    filename = os.path.basename(pathing)
    filename_no_ext = os.path.splitext(filename)[0]
    parts = filename_no_ext.split('_')
    kunjdate = parts[-1] 

    output_path = os.path.join(output_dir, f'CoolKidz_Charges_of_{kunjdate}.xlsx')
