...
def apply_logic_FOR_AR_Gestro(pathing1, pathing2):
    ...
    resultant_data["Provider Group NPI"] = 1891737532
    resultant_data["Provider Group Name"] = "Gastroenterology Atlanta"
    ...
    ensure_directory_exists("Results/AR/AR_Gastro/Reference Files")
    resultant_data.to_excel("Results/AR/AR_Gastro/Reference Files/Reference_AR_Gastro_File.xlsx", index=False)
    generated_files.append("Results/AR/AR_Gastro/Reference Files/Reference_AR_Gastro_File.xlsx")
    ...
    # Desized Column Ordering with respect to RAIO Gastro AR
    ...
    ensure_directory_exists('Results/AR/AR_Gastro/Insurances_Above_120')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_above_120:
        ...
        if not group_same.empty:
            filename_same = "Gastro_120+_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gastro/Insurances_Above_120/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Above_120/{filename_same}')

        # Processing the 'Different' group
        if not group_different.empty:
            filename_different = "Gastro_120+_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gastro/Insurances_Above_120/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Above_120/{filename_different}')
    ...
    ensure_directory_exists('Results/AR/AR_Gastro/Insurances_Between_91_To_120')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_91_to_120:
        ...
        if not group_same.empty:
            filename_same = "Gastro_91_To_120_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gastro/Insurances_Between_91_To_120/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Between_91_To_120/{filename_same}')

        if not group_different.empty:
            filename_different = "Gastro_91_To_120_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gastro/Insurances_Between_91_To_120/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Between_91_To_120/{filename_different}')
    ...
    ensure_directory_exists('Results/AR/AR_Gastro/Insurances_Between_61_To_90')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_61_to_90:
        ...
        if not group_same.empty:
            filename_same = "Gastro_61_To_90_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gastro/Insurances_Between_61_To_90/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Between_61_To_90/{filename_same}')

        if not group_different.empty:
            filename_different = "Gastro_61_To_90_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gastro/Insurances_Between_61_To_90/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Between_61_To_90/{filename_different}')
    ...
    ensure_directory_exists('Results/AR/AR_Gastro/Insurances_Between_31_To_60')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_between_31_to_60:
        ...
        if not group_same.empty:
            filename_same = "Gastro_31_To_60_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gastro/Insurances_Between_31_To_60/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Between_31_To_60/{filename_same}')

        if not group_different.empty:
            filename_different = "Gastro_31_To_60_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gastro/Insurances_Between_31_To_60/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Between_31_To_60/{filename_different}')
    ...
    ensure_directory_exists('Results/AR/AR_Gastro/Insurances_Below_30')  # Ensure the directory exists
    ...
    for insurance_name, group in insurance_groups_less_30:
        ...
        if not group_same.empty:
            filename_same = "Gastro_30-_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Primary" + ".xlsx"
            group_same.to_excel(f'Results/AR/AR_Gastro/Insurances_Below_30/{filename_same}', index=False)
            counter += 1
            # print(f"Saved {filename_same} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Below_30/{filename_same}')

        if not group_different.empty:
            filename_different = "Gastro_30-_" + insurance_name.replace('/', '_').replace('\\', '_') + f"_As of {date}_Partial" + ".xlsx"
            group_different.to_excel(f'Results/AR/AR_Gastro/Insurances_Below_30/{filename_different}', index=False)
            counter += 1
            # print(f"Saved {filename_different} - Group {counter}")
            generated_files.append(f'Results/AR/AR_Gastro/Insurances_Below_30/{filename_different}')



    resultant_data_above_120.to_excel('Results/AR/AR_Gastro/Reference Files/Data_Above_120.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gastro/Reference Files/Data_Above_120.xlsx')  # Append file path to generated_files
    resultant_data_between_91_and_120.to_excel('Results/AR/AR_Gastro/Reference Files/Data_Between_91_To_120.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gastro/Reference Files/Data_Between_91_To_120.xlsx')  # Append file path to generated_files
    resultant_data_between_61_and_90.to_excel('Results/AR/AR_Gastro/Reference Files/Data_Between_61_To_90.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gastro/Reference Files/Data_Between_61_To_90.xlsx')  # Append file path to generated_files
    resultant_data_between_31_and_60.to_excel('Results/AR/AR_Gastro/Reference Files/Data_Between_31_To_60.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gastro/Reference Files/Data_Between_31_To_60.xlsx')  # Append file path to generated_files
    resultant_data_less_30.to_excel('Results/AR/AR_Gastro/Reference Files/Data_Less_30.xlsx', index = False)
    generated_files.append('Results/AR/AR_Gastro/Reference Files/Data_Less_30.xlsx')  # Append file path to generated_files


    # Splitting the entire dataset according to the number of days allocated and saving them
    # time_frames = {
    ...
    #     folder_path = f"Results/AR/AR_Gastro/{value['folder']}"
    ...
    zip_file_path = 'temp/AR_Gastro_output_files.zip'
    os.makedirs(os.path.dirname(zip_file_path), exist_ok=True)
    with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for file_path in generated_files:
            archive_path = os.path.relpath(file_path, 'Results/AR/AR_Gastro')
...
# print(AR_Gastro_Preprocessing(pathing = "31.09 - Payer Claim Aging - Detail (1).xlsx"))